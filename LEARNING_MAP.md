...
#### هل الأساس مستقرّ بما يكفي للانتقال إلى Study؟ — **نعم، بعد نصف يوم**

مُتحقَّق منه عند Commit `7aeac1e`:

| المحور | الحالة | يمنع؟ |
|--------|--------|:-----:|
| المصادقة (الأنواع الثلاثة تسجّل دخولها) | مستقرّ ✅ | ❌ |
| الهيكل الأكاديمي CRUD + فلترة + نطاق | مستقرّ ومتّسق عبر الكيانات الخمسة ✅ | ❌ |
| قاعدة البيانات والهجرات | مستقرّة، هجرة واحدة نظيفة ✅ | ❌ |
| **بدائية النطاق `IsWithinScope`** | **معطوبة: حارس ناقص + 12 موضع استدعاء** | ✅ **نعم** |
| **انتماء `Semester` للمستأجر** | **غير محسوم — لا `UniversityId`** | ✅ **نعم** |
| بذور الأدوار (`BatchAdmin` / `Lecturer`) | غير محسومة | ⚠️ قبل `StudentResult` |
| العقود (409 يُرجَع 400، `Add*` تُرجع 200) | غير مطابقة للمعلَن | ❌ مزعج لا مانع |
| الواجهة الأمامية (`wwwroot` في المكان الخطأ) | غير مخدومة | ❌ مستقلّة |
| الاختبارات | صفر | ❌ لا يمنع تقنياً — لكنه يضاعف كلفة ما بعده |

> **الخلاصة:** الأجزاء المبنية **مكتملة ومستقرّة وظيفياً**. المانع الوحيد ليس نقصاً في الميزات، بل **بدائية أمنية مشتركة معطوبة + قرار تصميمي واحد غير محسوم** — انظر البوّابة في القسم ٣.
...
### ⛔ بوّابة قبل البدء — بندان يمنعان الانتقال إلى Study

**لا تكتب سطراً واحداً في وحدة Study قبل إغلاق هذين البندين.** السبب ليس كمالياً:

| # | البند | لماذا يمنع | الكلفة الآن ← بعد Study |
|:-:|------|-----------|:----------------------:|
| أ | **بدائية النطاق معطوبة** — حارس `IsWithinScope` يفحص 3 حقول من 4، و12 موضع استدعاء تُغفل الوسيط الرابع | Study ستضيف `SubjectOwnerHandler` و`SemesterOwnerHandler` و`StudentResultOwnerHandler` — **ستنسخ نفس النمط المعطوب ثلاث مرات** | ساعة ← ثلاثة أضعاف |
| ب | **`Semester` بلا `UniversityId`** — الفصول عالمية عبر كل الجامعات | تكسر سلسلة النطاق: **لا يمكن بناء `SemesterOwnerHandler` لأن الفصل لا ينتمي لأحد**. قرار تصميمي لا إصلاح | مجاني (لا كود يعتمد عليه) ← هجرة مؤلمة |

> **البند (ب) هو أهم قرار تصميمي مفتوح في المشروع كله.** ثلاثة خيارات: (أ) `UniversityId` على الفصل — الأصحّ لنظام متعدد المستأجرين · (ب) إبقاؤه عالمياً بإدارة `Super Admin` فقط · (ج) ربطه بـ `College`.
> **احسمه قبل أول هجرة، لأنه اليوم مجاني تماماً.**

**وسؤال ثالث بلا جواب اليوم — احسمه قبل `StudentResult`:** *مَن يُدخِل الدرجات؟* دور `Lecturer` مزروع بلا أي `[Authorize]`، و`SectionSubject.LecturerName` **نصّ حرّ لا مفتاح أجنبي**، ودور `BatchAdmin` مذكور في 15 موضعاً وغير مزروع أصلاً. الإجابة تحدد تصميم الوحدة كلها.
...
الآن ──────────────────────────────────────────────────────────►
  ...
  0. ⛔ البوّابة: حارس IsWithinScope + جرد الـ 12 موضعاً        ← ساعة، إلزامي
  0.5 ⛔ قرار UniversityId على Semester + الهجرة                ← إلزامي، مجاني الآن
  0.7 الفهارس الفريدة الثلاثة (في نفس الهجرة)                  ← ١٠ دقائق
  0.9 3 اختبارات xUnit على IsWithinScope                       ← نصف يوم، تمنع تكرار ما حدث

  1. StudentResult: ورث BaseEntity + أصلح Enterd            ← ١٠ دقائق، يمهّد لما بعده
  2. StudentResult: SetGrades بحدود 0–100                   ← أول كيان غني أكتبه بيدي
...
**الفهارس الفريدة الثلاثة الناقصة** (بند 0.7) — وهي فجوة لافتة لأن المشروع **مُفهرَس بامتياز في كل مكان آخر**؛ `Subject` وحده فيه فهرسان مركّبان فريدان:

| الكيان | الفهرس المطلوب | ما يمنعه |
|--------|----------------|----------|
| `SectionSubject` | `(SectionId, SubjectId, SemesterId)` | إسناد نفس المادة لنفس الشعبة في نفس الفصل مرتين |
| `StudentResult` | `(StudentId, SectionSubjectId)` | تسجيل درجتين لنفس الطالب في نفس المادة |
| `Semester` | `IsCurrent` فريد مُصفّى `.HasFilter("[IsCurrent] = 1")` | خمسة فصول "حالية" معاً |

> **لاحظ:** هذه ليست جهلاً بالفهرسة — بل **نفس نمط "طبّقتُ القاعدة على الكيان الذي فكّرتُ فيه ونسيتُ جيرانه"**. الفارق أنك تكتشفها الآن قبل وجود أي بيانات — أرخص لحظة ممكنة.

**البند ٢ هو نقطة التحوّل الحقيقية** — أول مرة أكتب كياناً يحرس نفسه من الصفر، بقراري لا نقلاً. **وانتبه:** المطلوب حراسة نطاق الدرجة (0–100)، **لا** إعادة حساب `Total` — فهو عمود محسوب مخزَّن في SQL بالفعل، وذلك أقوى من حسابه في الكيان.
...
> **أساس المعايرة:** النِّسب مبنية على `DEVELOPER_PROFILE.md` — تصنيف **Mid-level مبتدئ في كل المحاور عدا الاختبارات** (تُرقِّي من "Strong Junior" بعد جولة `7aeac1e`).
> **ليست** مقارنة بمعايير Senior Architect، ولا بمشروع ABP ناضج بفريق كامل.
> كل نسبة مقترنة بدليل من الكود، لا بانطباع.
>
> **آخر إعادة تقييم:** 2026-08-07 عند Commit `7aeac1e` — بعد إغلاق ثلاثة بنود 🔴 ونزول تحذيرات البناء من 10 إلى 3.
...
| 11 | **نطاق متعدد المستأجرين** | Tenant من ABP | مطالبات JWT مخصّصة | **95%** | **65%** ▲5 | **30** |
| 12 | **المصادقة** | OpenIddict + ABP Identity | JWT + Refresh Rotation + BCrypt | **95%** | **90%** ▲30 | 5 |
| 13 | **التسجيل (Logging)** | Serilog عبر الطبقات | `ILogger` في الـ Middleware فقط | **70%** | **40%** | 30 |
| 14 | **التفويض** | صلاحيات شجرية | أدوار + `OwnershipRequirement` | **95%** | **80%** ▲25 | 15 |

**المتوسط:** القرار التصميمي **≈ 90%** · دقة التنفيذ **≈ 79%** (كان 74%) · **الفجوة ≈ 11 نقطة** (كانت 16)

> ضاقت الفجوة **5 نقاط في جولة واحدة** — أكبر قفزة مرصودة. مصدرها ثلاثة إصلاحات حرجة، لا ميزة جديدة.
>
> **ما بقي من الـ 11 نقطة ليس معرفياً — بل انضباط الجرد:** أكبر بندين متبقيين (النطاق 30، التسجيل 30) أحدهما يُحلّ بسطر واحد + جرد `grep`، والآخر بيومين عمل.

---

### الأدلة وراء النِّسب المنخفضة

#### التفويض — 95% تصميم / **80%** تنفيذ ▲25
...
**ما أُغلق في جولة `7aeac1e`:**

| النقطة | الحالة |
|--------|--------|
| `DeleteSection` | ✅ **أُصلح** — تستخدم `SectionOwnerPolicy` + `AuthorizeAsync` |
| `GetAll*` في Department/Batch/Section | ✅ **أُصلح** — كلها الآن `[Authorize(Roles = "Super Admin")]` |
| `GetAll*` في College/University | ✅ كانت مُقيَّدة أصلاً |

**القاعدة صارت موحّدة عبر الكيانات الخمسة:** `GetAll*` = Super Admin · `Get*PerParent` = بقية الأدوار مع `scope`. قاعدة من سطر واحد، أصعب على النسيان.

**لماذا 80% لا 95%؟** لأن التغطية اكتملت على مستوى *نقاط النهاية* لكن **بدائية النطاق نفسها معطوبة** — انظر الفقرة التالية.

#### المصادقة — 95% تصميم / **90%** تنفيذ ▲30 (أكبر قفزة)

**التصميم:** تدوير Refresh Token مع تخزين الـ Hash فقط (SHA-256) وربط `ReplacedByTokenId`.

**ما أُصلح:** عطل `TokenUserInfoMapper:29,70` الذي كان يمنع تسجيل دخول **كل حسابات الموظفين**.

**وكيف أُصلح — وهذا هو المهم:** لم تكتفِ بإضافة فحص `null`؛ بل أضفت **مُميِّزاً صريحاً** `UserType Type` إلى `UserTypeResult` (`UserTypeResult.cs:17`) وبنيت التفريع عليه بدل الاستنتاج من "أي حقل ليس فارغاً".

> **هذا بالضبط مفهوم "الاتحاد المُميَّز" (Discriminated Union) — وهو المفهوم رقم ⑧ في القسم ٢ (انتقالات الحالة) مطبَّقاً على النوع بدل الحالة.** طبّقته بغريزتك قبل أن تقرأه. النقطة العامة: **حين يحمل نوعٌ أكثر من شكل، اجعل الشكل حقلاً صريحاً لا استنتاجاً من الفراغات.** هذا يقتل فئة الخطأ كلها لا الحالة الواحدة.

#### التحقق من المدخلات — 90% تصميم / 65% تنفيذ (بلا تغيير — الجولة الثالثة)
...
#### نطاق متعدد المستأجرين — 95% تصميم / **65%** تنفيذ ▲5 فقط

`ToUserScope()` صار يُسنِد `BatchId` ✅ (`ControllersExtensions.cs:109`). **لكن النسبة ارتفعت 5 نقاط فقط، والسبب درسٌ يستحق الوقوف عنده.**

**ما حدث بالضبط:** الجولة السابقة حذّرت حرفياً: *«الترتيب إلزامي: أصلح الحارس أولاً، ثم `ToUserScope`»*. **نُفِّذ العكس** — أُضيف `BatchId` وبقي الحارس (`EmployeeScopeExtension.cs:14`) يفحص ثلاثة حقول من أربعة.

**لماذا لم تنفجر الثغرة:** لأن `StudentAuthorizationInfo` يستخدم `int` غير قابل للـ null في `CollegeId` و`DepartmentId`، فلا يوجد اليوم مُوكَّل يحمل `BatchId` وحده.

> **هذه حماية عرضية من خيار نوع، لا حماية تصميمية.** الفرق جوهري: الحماية العرضية تختفي بأي تعديل غير ذي صلة.

**والأهم — الإصلاح أنتج عيباً جديداً:** `IsWithinScope` تفحص `BatchId` **أولاً**، والوسيط الرابع اختياري بافتراض `null`. فأي موضع استدعاء يُغفله يُرجع `false` لكل مستخدم مرتبط بدفعة.

**الجرد:** 21 موضع استدعاء · 9 تمرّر 4 وسائط ✅ · **12 تمرّر 3 أو أقل** ❌ — ومنها `SectionService.cs:50` (`AddSection`) بينما `BatchAdmin` صار مُصرَّحاً له على `CreateSection` في نفس الـ Commit.

> **الدرس المنقول (وهو أهم من العيب نفسه):**
> **حين تُعدِّل مُنتِجاً، اجرد مستهلكيه — لا سماتهم فقط.** راجعتَ سمات `[Authorize]` بعناية وأضفت `BatchAdmin` إلى 5 نقاط، لكنك لم تُشغِّل `grep -rn "IsWithinScope("`.
>
> هذا هو **نمط §3.1 في `DEVELOPER_PROFILE.md` بأدقّ صوره حتى الآن**: الوحدة المنسيّة تصغر كل جولة — من "كيان" إلى "نقطة نهاية" إلى **"وسيط رابع"**. الجذر واحد، والعلاج واحد: اختبار وحدة على `IsWithinScope` كان سيلتقط الحالتين معاً.
...
| 4 | **الكيان الغني** | كل الكيانات | `Total` بـ `private set` **+ عمود محسوب مخزَّن في SQL** — الحساب مُنفَّذ، والحارس على النطاقات غائب | **25%** | `StudentResult.SetGrades` (حدود 0–100) |
...
> **تصحيح على البند 4 (كان مكتوباً 10%):** التحقق من `StudentResultConfiguration.cs` أظهر أن `Total` **عمود محسوب مخزَّن فعلاً** — `HasComputedColumnSql("[Midterm] + [Practical] + [Final]", stored: true)`.
>
> أي أن الحساب **مُنفَّذ ومضمون على مستوى قاعدة البيانات** — وهذا **أقوى** من حسابه في دالة داخل الكيان: يستحيل أن يفتقد الاتساق مهما كان مصدر الكتابة، ويمكن الفهرسة عليه. **لا تستبدله بـ `SetGrades` تحسب `Total`.**
>
> **الفجوة الحقيقية إذن ليست الحساب — بل حراسة الثوابت:** `HasPrecision(5,2)` يسمح بـ `Midterm = 999.99`. الكيان الغني هنا يعني **رفض درجة خارج 0–100**، لا إعادة حساب المجموع. هذا هو ما تكتبه في `SetGrades`.
...
| المفهوم | جولة `25c99d2` | **اليوم (`7aeac1e`)** | الهدف | كيف أصل |
|---------|:--------------:|:---------------------:|:-----:|---------|
| التفويض | 55% | **80%** ▲25 | 90% | جرد وسائط `IsWithinScope` الـ 12 |
| المصادقة | 60% | **90%** ▲30 | 95% | لا شيء عاجل — مكتمل عملياً |
| النطاق | 60% | **65%** ▲5 | 90% | الحارس (سطر واحد) + الوسيط الرابع |
| التحقق | 65% | **65%** → | 85% | قاعدة `StudentNumber` — **معلَّقة 3 جولات** |
| التسجيل | 40% | **40%** → | 75% | `ILogger` في طبقة الخدمات |
| الكيان الغني | 10%\* | **25%** | 70% | `SetGrades` بحدود 0–100 (لا إعادة حساب `Total`) |
| خدمة المجال | 0% | **0%** → | 60% | `SectionSubjectManager` |
| الاختبارات | 0% | **0%** → | 40% | 3 اختبارات على `IsWithinScope` |

\* الرقم السابق كان مبنياً على قراءة ناقصة — انظر التصحيح في القسم (ب).

**قراءة الجدول:** ثلاثة محاور قفزت، وأربعة لم تتحرك. **المحاور الأربعة الساكنة كلها من نوع "عمل ممل صغير"** (قاعدة تحقق، سطر `ILogger`، مشروع اختبارات) — بينما القافزة كلها كانت "عيوباً حرجة مُبلَّغاً عنها".

> **التشخيص السلوكي:** الاستجابة للبلاغ المحدَّد **ممتازة**؛ المبادرة إلى البند الصغير غير المُلِحّ **ضعيفة**. الحلّ ليس عزيمة أكبر — بل **CI يرفض الدمج**، فيتحوّل البند الصغير من "اختيار" إلى "شرط".
